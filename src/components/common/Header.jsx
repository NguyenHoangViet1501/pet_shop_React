...
const Header = () => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.trim()) {
        try {
          const response = await productsApi.getProducts({ search: searchQuery, size: 5 });
              ...
              {showDropdown && searchResults.length > 0 && (
                <div className="dropdown-menu show w-100" style={{
                  ...
                  {searchResults.map(product => {
                     ...
                     const imageUrl = primaryImage?.imageUrl || product.productImage?.[0]?.imageUrl || 'https://via.placeholder.com/50';
                     ...
                     return (
                      ...
                      >
                        ...
                        <div className="flex-grow-1 overflow-hidden">
                          <div className="text-truncate fw-bold" style={{fontSize: '0.9rem'}}>{product.name}</div>
